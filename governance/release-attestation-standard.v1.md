...
1. Release attestation payloads SHALL be signed with `Ed25519`.
2. Signing key identifier SHALL be recorded in the artifact and SHALL reference a published key in the governance keyring.
3. Signature verification SHALL be reproducible from payload + published public key material; shared-secret-only verification is insufficient.
...
Attestations SHALL be machine-verifiable using the published attestation schema, published governance keyring, and verification script.
