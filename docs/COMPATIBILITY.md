...
- `signatureV3` may be present in payloads but does not satisfy trusted-signature requirements in `v46`.
...
- Groth16 fields are required in `v46`; missing fields fail closed.
- Only real `g16:` Groth16 payloads are accepted in `v46`.
- Default `v46` UI does not collect an optional `/v/...` path value.
