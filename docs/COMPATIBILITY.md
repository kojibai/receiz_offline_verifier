...
- `signatureV3` may be present in payloads but does not satisfy trusted-signature requirements in `v30`.
...
- Groth16 fields are required in `v30`; missing fields fail closed.
- Only real `g16:` Groth16 payloads are accepted in `v30`.
- Default `v30` UI does not collect an optional `/v/...` path value.
