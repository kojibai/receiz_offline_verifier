...
- `signatureV3` may be present in payloads but does not satisfy trusted-signature requirements in `v29`.
...
- Groth16 fields are required in `v29`; missing fields fail closed.
- Only real `g16:` Groth16 payloads are accepted in `v29`.
- Default `v29` UI does not collect an optional `/v/...` path value.
