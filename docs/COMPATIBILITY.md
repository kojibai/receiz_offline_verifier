...
- `signatureV3` may be present in payloads but does not satisfy trusted-signature requirements in `v27`.
...
- Groth16 fields are required in `v27`; missing fields fail closed.
- Only real `g16:` Groth16 payloads are accepted in `v27`.
- Default `v27` UI does not collect an optional `/v/...` path value.
