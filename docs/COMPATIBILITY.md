...
- `signatureV3` may be present in payloads but does not satisfy trusted-signature requirements in `v28`.
...
- Groth16 fields are required in `v28`; missing fields fail closed.
- Only real `g16:` Groth16 payloads are accepted in `v28`.
- Default `v28` UI does not collect an optional `/v/...` path value.
