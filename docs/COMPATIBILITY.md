...
- `signatureV3` may be present in payloads but does not satisfy trusted-signature requirements in `v26`.
...
- Groth16 fields are required in `v26`; missing fields fail closed.
- Only real `g16:` Groth16 payloads are accepted in `v26`.
- Default `v26` UI does not collect an optional `/v/...` path value.
