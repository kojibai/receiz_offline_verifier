...
Status: Canonical reference from `v70.0.0`; carried forward for `v105.0.0`
...
For `v70.0.0` and carried forward through `v92.0.0`, deterministic public snapshots and private account snapshots may also be explicit offline truth carriers when the response opts into persistence with `x-receiz-offline-persist`.
