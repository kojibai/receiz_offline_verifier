...
This document defines the verifier-visible data contract for `v55.1.0`.
...
Verified outcomes in `v55` require effective anchor context.
...
`v55` requires trusted Signature v4 verification for verified outcomes.
...
- `signatureV3` may be present for compatibility but does not satisfy trusted-signature success in `v55`.

## Receiz Signature v3 (legacy payload support)

`signatureV3` payload shape is retained in schema for compatibility.
In `v55`, Signature v3 verification and key-policy evaluation are not executed, and `signatureV3` does not satisfy trusted-signature requirements.
...
## Groth16 requirements (`v55`)
`v55` requires Groth16 artifact fields and accepts only real-mode proof payloads.
...
Note: the default `v55` UI does not prompt for a manual `/v/...` path input.
